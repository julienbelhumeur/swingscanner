    ...
    with col_email:
        if st.button("📧  Envoyer par email", use_container_width=True):
            try:
                ...
                rows_html = ""
                def fmt_val(val, decimals=2, suffix=""):
                    try:
                        import math
                        v = float(val)
                ...
                for _, r in df.head(20).iterrows():
                    ...
                    entree = fmt_val(r["Entrée suggérée"])
                    stop_v = fmt_val(r["Stop-loss"])
                    cible  = fmt_val(r["Cible"])
                    rr_v   = fmt_val(r["Ratio R/R"], 1, ":1")
                    rows_html += f"<tr style='border-bottom:1px solid #1e2230;'><td style='padding:8px 12px;color:#e2e8f0;font-weight:600;'>{r['Ticker']}</td><td style='padding:8px 12px;color:#94a3b8;font-size:12px;'>{r['Nom'][:25]}</td><td style='padding:8px 12px;color:{dc};'>{r['Direction']}</td><td style='padding:8px 12px;color:{sc};font-weight:600;'>{r['Score']}/10</td><td style='padding:8px 12px;color:#93c5fd;'>{r['Prix']:.2f}</td><td style='padding:8px 12px;color:#93c5fd;'>{entree}</td><td style='padding:8px 12px;color:#f87171;'>{stop_v}</td><td style='padding:8px 12px;color:#4ade80;'>{cible}</td><td style='padding:8px 12px;color:#fbbf24;'>{rr_v}</td></tr>"
