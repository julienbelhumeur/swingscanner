...
st.markdown("<div class='section-header'>Recherche d'un titre</div>", unsafe_allow_html=True)
col_search, col_btn = st.columns([3, 1])
with col_search:
    search_ticker = st.text_input(
        "Ticker",
        placeholder="Ex: GLDX.TO, AAPL, RY.TO, SPY…",
        label_visibility="collapsed",
        key="search_ticker_input"
    )
with col_btn:
    search_btn = st.button("🔍 Analyser", use_container_width=True, key="search_btn")

if search_btn and search_ticker:
    ticker_clean = search_ticker.strip().upper()
    with st.spinner(f"Analyse de {ticker_clean}…"):
        from scanner import fetch_ticker_data, get_ticker_info
        df_t = fetch_ticker_data(ticker_clean)
        if df_t is None or df_t.empty:
            st.error(f"Impossible de trouver les données pour {ticker_clean}. Vérifie le symbole (ex: RY.TO pour TSX, AAPL pour NYSE).")
        else:
            from indicators import compute_indicators, score_setup
            df_t = compute_indicators(df_t)
            score, direction_signal, signals, entry, stop, target = score_setup(df_t, indicators_config)
            last = df_t.iloc[-1]
            prev = df_t.iloc[-2]
            current_price = float(last["Close"])
            variation = ((current_price - float(prev["Close"])) / float(prev["Close"])) * 100
            avg_vol = float(df_t["Volume"].rolling(20).mean().iloc[-1])
            curr_vol = float(last["Volume"])
            vol_ratio = curr_vol / avg_vol if avg_vol > 0 else 1.0
            rr = abs(target - entry) / abs(entry - stop) if (entry is not None and stop is not None and target is not None and abs(entry - stop) > 0) else 0.0
            info = get_ticker_info(ticker_clean)
            dir_class = "signal-bull" if "Hausse" in direction_signal else "signal-bear" if "Baisse" in direction_signal else "signal-neut"
            score_color = "#4ade80" if score >= 8 else "#fbbf24" if score >= 6 else "#94a3b8"

            st.markdown("---")
            col_chart, col_info = st.columns([2, 1])
            with col_chart:
                from charts import build_chart
                fig = build_chart(ticker_clean)
                if fig:
            ...
            with col_info:
                ...
                <div class='metric-card' style='margin-bottom:12px;'>
                    <div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;'>
                        <span style='font-size:20px; font-weight:600; font-family:IBM Plex Mono;'>{ticker_clean}</span>
                        <span class='signal-badge {dir_class}'>{direction_signal}</span>
                    </div>
                    <div style='font-size:13px; color:#64748b; margin-bottom:16px;'>{info["name"][:35]}</div>
                    <div style='display:grid; grid-template-columns:1fr 1fr; gap:12px;'>
                        <div>
                            <div class='metric-label'>Prix actuel</div>
                            <div style='font-size:20px; font-weight:600; font-family:IBM Plex Mono;'>{current_price:.2f}</div>
                            <div style='font-size:12px; color:{"#4ade80" if variation>0 else "#f87171"};'>{variation:+.2f}%</div>
                        </div>
                        <div>
                            <div class='metric-label'>Score setup</div>
                            <div style='font-size:20px; font-weight:600; font-family:IBM Plex Mono; color:{score_color};'>{score}/10</div>
                            <div class='score-bar-bg'><div class='score-bar-fill' style='width:{score*10}%; background:{score_color};'></div></div>
                        </div>
                    </div>
                </div>
                <div class='metric-card' style='margin-bottom:12px;'>
                    ...
                    <div style='display:grid; gap:8px; font-family:IBM Plex Mono; font-size:13px;'>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>Entrée suggérée</span><span style='color:#93c5fd;'>{f"{entry:.2f}" if entry is not None else "—"}</span></div>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>Stop-loss</span><span style='color:#f87171;'>{f"{stop:.2f}" if stop is not None else "—"}</span></div>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>Cible (TP)</span><span style='color:#4ade80;'>{f"{target:.2f}" if target is not None else "—"}</span></div>
                        <div style='border-top:1px solid #1e2230; padding-top:8px; display:flex; justify-content:space-between;'><span style='color:#64748b;'>Ratio R/R</span><span style='color:#fbbf24; font-weight:500;'>{f"{rr:.1f}:1" if rr else "—"}</span></div>
                    </div>
                </div>
                <div class='metric-card'>
                    <div class='metric-label' style='margin-bottom:10px;'>Indicateurs</div>
                    <div style='display:grid; gap:8px; font-family:IBM Plex Mono; font-size:13px;'>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>RSI (14)</span><span style='color:{"#4ade80" if float(last["RSI"])<35 else "#f87171" if float(last["RSI"])>65 else "#e2e8f0"};'>{float(last["RSI"]):.1f}</span></div>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>MACD</span><span style='color:#e2e8f0;'>{"Haussier" if float(last["MACD_hist"])>0 else "Baissier"}</span></div>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>Volume ratio</span><span style='color:{"#fbbf24" if vol_ratio>1.5 else "#e2e8f0"};'>{vol_ratio:.1f}x</span></div>
                        <div style='display:flex; justify-content:space-between;'><span style='color:#64748b;'>MA20 vs Prix</span><span style='color:{"#4ade80" if current_price>float(last["MA20"]) else "#f87171"};'>{"Au-dessus" if current_price>float(last["MA20"]) else "En-dessous"}</span></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                if signals:
                    ...
                    <div class='metric-card' style='margin-top:12px;'>
                        ...
                        <div style='font-size:12px; color:#94a3b8; line-height:1.8;'>{signals}</div>
...
if df is None:
    ...
    styled = display_df.style\
        ...
        .format({
            "Prix": lambda x: f"{x:.2f}" if x is not None and str(x) != 'nan' else "—",
            "Variation %": lambda x: f"{x:+.2f}%" if x is not None and str(x) != 'nan' else "—",
            "RSI": lambda x: f"{x:.1f}" if x is not None and str(x) != 'nan' else "—",
            "Volume ratio": lambda x: f"{x:.1f}x" if x is not None and str(x) != 'nan' else "—",
            "Entrée suggérée": lambda x: f"{x:.2f}" if x is not None and str(x) != 'nan' else "—",
            "Stop-loss": lambda x: f"{x:.2f}" if x is not None and str(x) != 'nan' else "—",
            "Cible": lambda x: f"{x:.2f}" if x is not None and str(x) != 'nan' else "—",
        })\
        .set_properties(**{
        ...
        if st.button("📧  Envoyer par email", use_container_width=True):
            try:
                ...
                for _, r in df.head(20).iterrows():
                    ...
                    rows_html += f"<tr style='border-bottom:1px solid #1e2230;'><td style='padding:8px 12px;color:#e2e8f0;font-weight:600;'>{r['Ticker']}</td><td style='padding:8px 12px;color:#94a3b8;font-size:12px;'>{r['Nom'][:25]}</td><td style='padding:8px 12px;color:{dc};'>{r['Direction']}</td><td style='padding:8px 12px;color:{sc};font-weight:600;'>{r['Score']}/10</td><td style='padding:8px 12px;color:#93c5fd;'>{r['Prix']:.2f}</td><td style='padding:8px 12px;color:#93c5fd;'>{f\"{r['Entrée suggérée']:.2f}\" if r['Entrée suggérée'] is not None and str(r['Entrée suggérée']) != 'nan' else '—'}</td><td style='padding:8px 12px;color:#f87171;'>{f\"{r['Stop-loss']:.2f}\" if r['Stop-loss'] is not None and str(r['Stop-loss']) != 'nan' else '—'}</td><td style='padding:8px 12px;color:#4ade80;'>{f\"{r['Cible']:.2f}\" if r['Cible'] is not None and str(r['Cible']) != 'nan' else '—'}</td><td style='padding:8px 12px;color:#fbbf24;'>{f\"{r['Ratio R/R']:.1f}:1\" if r['Ratio R/R'] is not None and str(r['Ratio R/R']) != 'nan' else '—'}</td></tr>"
